...
public class FooIOTalonFX implements FooIO {
  ...
  public FooIOTalonFX() {
    foo0 = new TalonFXIO(Constants.CANIds.fooMotor0);              // RIO bus
    // CANivore bus: new TalonFXIO(CANIds.turretMotor, CANIds.CANivoreBus)
    configureFooMotor();
  }
...
public FooIOTalonFX() {
  foo0 = new TalonFXIO(Constants.CANIds.fooMotor0);
  foo1 = new TalonFXIO(Constants.CANIds.fooMotor1);
...
| Value | Home | Naming |
|---|---|---|
| CAN ID | `Constants.CANIds` | `camelCase` — `flywheelMotor0` |
| CANivore device | `Constants.CANIds` + pass `CANIds.CANivoreBus` | `camelCase` |
...
There is no DIO convention yet. `Constants` has an empty `DigitalOutputs` class under a `/* Digital Ports */` comment; it's misnamed for sensor inputs. Add a sibling `DigitalInputs` class rather than putting an input port in `DigitalOutputs`, and follow `CANIds`' camelCase since it's an ID registry.
...
```
- [ ] CAN IDs are free — check Constants.CANIds AND TunerConstants (drive uses 1-8, mechanisms 14-28)
...
- [ ] CAN ID in Constants.CANIds (camelCase), reduction in PhysicalConstants, tuning in FooConstants
