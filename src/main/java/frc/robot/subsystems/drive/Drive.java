...
public class Drive extends ExpandedSubsystem implements PowerManaged {
  ...
  public Drive(
    ...
    if (Constants.getMode() == Mode.SIM) {
      RobotContainer.simState.initializeUnderlyingPoseEstimator(kinematics, rawGyroRotation, lastModulePositions);
    }
    ...
    PathPlannerLogging.setLogActivePathCallback(
        (activePath) -> {
          Logger.recordOutput("RobotState/Trajectory", activePath.toArray(new Pose2d[0]));
        });
  ...
  public void earlyPeriodic() {
    EpochTimer.BeginEpoch("Drive");
    {
      // Update odometry before the rest of the robot code
