...
public class TagCamera {
    ...
    private void drawDebugLines(RawFiducialRecord[] rawFiducials, int length) {
        ...
        Pose3d robotPose = new Pose3d(RobotContainer.driveSubsystem.getPose());
    ...
    */
    ...
    private Optional<TagPoseEstimate> filterMegatagEstimate() {
        ...
        if (Math.abs(RobotContainer.telemetry.getYawVelocityAtTimestamp(
        ...
        if (VisionConstants.USE_AUTOMATIC_STANDARD_DEVIATIONS) {
            ...
            double xStd = inputs.rawStandardDeviationArray[VisionConstants.kMegatag1XStdDevIndex] * quality;
            double yStd = inputs.rawStandardDeviationArray[VisionConstants.kMegatag1YStdDevIndex] * quality;
            double xyStd = Math.max(xStd, yStd);

            double yawStd = inputs.rawStandardDeviationArray[VisionConstants.kMegatag1YawStdDevIndex] * quality;
    ...
    private Optional<TagPoseEstimate> calculateGyroEstimate() {
        ...
        Matrix<N3, N1> estimationStdDevs;
        if (VisionConstants.USE_AUTOMATIC_STANDARD_DEVIATIONS) {
            double xStd = inputs.rawStandardDeviationArray[VisionConstants.kMegatag1XStdDevIndex];
            double yStd = inputs.rawStandardDeviationArray[VisionConstants.kMegatag1YStdDevIndex];
            double xyStd = Math.max(xStd, yStd);
