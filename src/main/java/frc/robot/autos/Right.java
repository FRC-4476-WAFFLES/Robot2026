...
public class Right extends SequentialCommandGroup {
  ...
  private final BlueRelativeTarget point2 = new BlueRelativeTarget(7.9, 6.0, Rotation2d.fromDegrees(-45));
  ...
  public Right() {
    ...
    addCommands(
        AutoUtils.resetOdometry(start.mirror()),

        DriveCommands.passThroughTarget(point1.mirror()),
        ...
        Commands.parallel(
            Commands.sequence(
                ...
                ShooterCommands.shootAutoCommand(10)
            ),
            collectBalls.follow()
        )
    );
